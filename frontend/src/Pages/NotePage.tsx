...
const NotePage = () => {
  ...
  const _createUser = () => {
    if (_formValidation()) {
      axios.post("http://ec2-3-7-254-6.ap-south-1.compute.amazonaws.com:5000/notes/new", noteFields).then((resp) => {
        if (resp) {
          console.log(resp);
  ...
  const [findNotes, setFindNotes] = useState("");
    const _getUser = (e: any) => {
      if (e.key === "Enter") {
        axios
          .get(`http://ec2-3-7-254-6.ap-south-1.compute.amazonaws.com:5000/notes/read/${findNotes}`)
          .then((res) => {
            if (res.data) {
  ...
  const _getUsers = () => {
    _handleReadAll();
    axios
      .get("http://ec2-3-7-254-6.ap-south-1.compute.amazonaws.com:5000/notes/readAll")
      .then((res) => {
        if (res.data) {
  ...
  const _updateUser = () => {
    axios
      .put(`http://ec2-3-7-254-6.ap-south-1.compute.amazonaws.com:5000/notes/update/${updateNotes}`, updateFields)
      .then((resp) => {
        if (resp) {
          console.log(resp);
          setUpdateToast(true);
  ...
  const _deleteUser = () => {
    axios
      .delete(`http://ec2-3-7-254-6.ap-south-1.compute.amazonaws.com:5000/notes/delete/${deleteNotes}`)
      .then((resp) => {
        if (resp) {
          console.log(resp);
          setDeleteToast(true);
