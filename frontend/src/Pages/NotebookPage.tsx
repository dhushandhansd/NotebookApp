...
const NotebookPage = () => {
  ...
  const _createUser = () => {
    if (_formValidation()) {
      axios
        .post(
          "http://3.108.136.43:5000/notebook/new",
          noteFields
        )
        .then((resp) => {
          if (resp) {
  ...
  const _getUser = (e: any) => {
    if (e.key === "Enter") {
      axios
        .get(
          `http://3.108.136.43:5000/notebook/read/${findNotes}`
        )
        .then((res) => {
          if (res.data) {
  ...
  const _getUsers = () => {
    ...
    axios
      .get(
        "http://3.108.136.43:5000/notebook/readAll"
      )
      .then((res) => {
        if (res.data) {
  ...
  const _updateUser = () => {
    axios
      .put(
        `http://3.108.136.43:5000/notebook/update/${updateNotes}`,
        updateFields
      )
      .then((resp) => {
        if (resp) {
  ...
  const _deleteUser = () => {
    axios
      .delete(
        `http://3.108.136.43:5000/notebook/delete/${deleteNotes}`
      )
      .then((resp) => {
        if (resp) {
          console.log(resp);
